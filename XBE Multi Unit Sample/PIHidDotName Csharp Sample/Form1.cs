...
namespace PIHidDotName_Csharp_Sample
{
    public partial class Form1 : Form, PIEDataHandler, PIEErrorHandler
    {
        ...
        ListBox thisListBox;
        //end thread-safe
        byte[][] lastdata = null; //need one of these arrays for each device found, first element is device, 2nd is that devices' data
        ...
        public void HandlePIEHidData(Byte[] data, PIEDevice sourceDevice, int error)
        {
            ...
            //if (sourceDevice == devices[selecteddevice])
            {
               //find handle
                ...
                String output = sourceDevice.ProductString + ", handle=" + thishandle.ToString() + ", pid=" + sourceDevice.Pid.ToString() + ", oem=" + sourceDevice.Version.ToString() + ", unit id=" + data[1].ToString() + ", data=";
                ...
                if (data[2] < 4)
                {
                    ...
                    //All this stuff is device dependent
                    //buttons
        ...
        private void BtnEnumerate_Click(object sender, EventArgs e)
        {
                ...
                for (int i = 0; i < devices.Length; i++)
                {
                    ...
                    if (devices[i].HidUsagePage == 0xc && devices[i].WriteLength>1)
                    {
                        switch (devices[i].Pid)
                        {
                            ...
                            //    listBox4.Items.Add(devices[i].ProductString +" (handle: "+i.ToString()+")");
                            ...
                            default:
                                ...
                                listBox4.Items.Add(devices[i].ProductString + ", handle= " + i.ToString() + ", pid= " + devices[i].Pid.ToString()+", oem= "+devices[i].Version.ToString());
                                cbotodevice[cbocount] = i;
                                cbocount++;
                    ...
                    else
                    {
                        ...
                        else if ((devices.Length==1) && (devices[i].HidUsage==6) && (devices[i].HidUsagePage==1))
                        {
                            MessageBox.Show(devices[i].ProductString+" in KVM mode");
                        }
                    }
                }
            }
            if (CboDevices.Items.Count > 0)
            {
                ...
                //lastdata = new byte[devices[selecteddevice].ReadLength];
                lastdata = new byte[CboDevices.Items.Count][];
                for (int i = 0; i < CboDevices.Items.Count; i++)
                {
                    int hnd=cbotodevice[i];
                    lastdata[i] = new byte[devices[hnd].ReadLength];
                }
                ...
                toolStripStatusLabel1.Text = devices.Count().ToString()+ "devices found";
            }
        ...
        private void ChkBank1OnOff_CheckedChanged(object sender, EventArgs e)
        {
            ...
            if (selecteddevice != -1) //do nothing if not enumerated
            {
                ...
                {
                    toolStripStatusLabel1.Text = "Write Success - All Bank 1 BL on(blue)/off";
                }
            }
        }
        ...
        private void ChkBank2OnOff_CheckedChanged(object sender, EventArgs e)
        {
            ...
            if (selecteddevice != -1) //do nothing if not enumerated
            {
                ...
                {
                    toolStripStatusLabel1.Text = "Write Success - All Bank 2 BL on(red)/off";
                }
            }
        }
        ...
        private void BtnPID3_Click(object sender, EventArgs e)
        {
            if (selecteddevice != -1) //do nothing if not enumerated
            {
                ...
                int newdevice = Convert.ToInt16(txtChangeDevice.Text);

                wData[0] = 0;
                wData[1] = 204; //0xcc
                wData[2] = (byte)newdevice;  //valid entries 0=pid 1, 1=pid 2, etc. Consult sdk docs for endpoints available on each device (pid)
        ...
        private void BtnNoChange_Click(object sender, EventArgs e)
        {
            ...
            if (selecteddevice != -1) //do nothing if not enumerated
            {
                ...
                try 
                {
                    defaultdevice = Convert.ToInt16(txtDefaultDevice.Text);
                }
                catch 
                {
                    MessageBox.Show("Invalid entry, enter integer 0=pid 1, 1=pid 2, etc.");
                }


                wData[0] = 0;
                wData[1] = 196; //0xc4
                wData[2] = (byte)defaultdevice; //this may not always be 0, Trackball and Joystick for example
        ...
        private void BtnChange_Click(object sender, EventArgs e)
        {
            if (selecteddevice != -1) //do nothing if not enumerated
            {
                ...
                try
                {
                    kvmdevice = Convert.ToInt16(txtKVMDevice.Text);
                }
                catch
                {
                    MessageBox.Show("Invalid entry, enter integer corresponding to the KVM pid. For XBE series this is 7, for XK series this is 1");
                }

                wData[0] = 0;
                wData[1] = 196; //0xc4
                wData[2] = (byte)kvmdevice;
        ...
        private void btnGetBLState_Click(object sender, EventArgs e)
        {
            ...
            if (selecteddevice != -1) //do nothing if not enumerated
            {
                ...
                int blindex = Convert.ToInt16(txtKeyIndexGet.Text); 

                wData[0] = 0;
                wData[1] = 167; //0xA7;
                wData[2] = (byte)blindex; 
                int result = 404;
        ...
        private void ChkBLOnOff_CheckStateChanged(object sender, EventArgs e)
        {
            ...
            if (selecteddevice != -1)
            {
                ...
                int iindex = Convert.ToInt16(txtBL.Text);
                ...
                if (bank == 1)
                {
                    ...
                    try
                    {
                        bloffset = Convert.ToInt16(txtBank2Offset.Text);
                    }
                    catch
                    {
                        MessageBox.Show("Invalid Bank 2 offset, please enter integer Bank 2 offset in textbox to the right of the Bank combobox selection, for example XK-24 has an offset of 24, XK-80 has an offset of 80");
                        return;
                    }
                    
                    iindex = iindex + bloffset;
        ...
        private void btnSetRGB_Click(object sender, EventArgs e)
        {
            ...
            if (selecteddevice != -1)
            {
                byte buttonindex = Convert.ToByte(txtRGBIndex.Text);
        ...
        private void CboBankLegacy_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (CboBankLegacy.SelectedIndex == 1)
            {
                MessageBox.Show("Must enter Bank 2 offset in shown textbox,overwrite the text with offset. For example XK-24 has an offset of 24, XK-80 has offset of 80.");
        ...
        {
            CboDevices.SelectedIndex = listBox4.SelectedIndex;
        }

        private void btnSaveBL2_Click(object sender, EventArgs e)
        {
